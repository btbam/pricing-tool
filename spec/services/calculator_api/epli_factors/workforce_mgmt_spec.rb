...
RSpec.describe CalculatorApi::EpliFactors::WorkforceMgmt do
  describe '#process_factor' do
    ...
    it 'defaults to 1 when renewal is nil' do
      factor = CalculatorApi::EpliFactors::WorkforceMgmt.new(company, sic, nil)
      expect(factor.factor).to be(1)
    end

    xit 'returns renewal when renewal is between 0.8 and 1.5' do
      allow(renewal).to receive(:workforce_mgmt).and_return(1.1)
      factor = CalculatorApi::EpliFactors::WorkforceMgmt.new(company, sic, renewal)
      expect(factor.factor).to be(1.1)
    end

    xit 'defaults to 1 when renewal is not between 0.8 and 1.5' do
      allow(renewal).to receive(:workforce_mgmt).and_return(2.2)
      factor = CalculatorApi::EpliFactors::WorkforceMgmt.new(company, sic, renewal)
      expect(factor.factor).to be(1)
    end
  end
